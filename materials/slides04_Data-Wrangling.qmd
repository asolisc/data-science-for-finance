...
```{r}
mutate(fang_data, daily_return = close/open -1)
```
...
```{r}
head(transform(fang_data, daily_return = close/open -1))
```
...
```{r}
mutate(fang_data, year = lubridate::year(date) )
```
...
```{r}
mutate(fang_data, security_type = "Stocks" )
```

# Create New Variables

mutate(data, condition) allows you to create new variables (columns) based on existing columns. You can even use newly created columns in the same step.

```{r}
mutate(fang_data, daily_return = close/open -1, return_sign = if_else(daily_return>0, "Positive", "Negative"))
```
...
```{r}
mutate(fang_data, daily_return = close/open -1, return_sign = if_else(daily_return>0, "Positive", "Negative"), .before = 1)
```

The . indicates that .before is an argument to the function, not the name of a third new variable we are creating. You can also use .after to add after a variable, and in both .before and .after you can use the variable name instead of a position. For example, we could add the new variables after close:

```{r}
mutate(fang_data, daily_return = close/open -1, return_sign = if_else(daily_return>0, "Positive", "Negative"), .after = close)
```
...
```{r}
mutate(fang_data, across(.cols = everything(), as.character))
```

```{r}
mutate(fang_data, across(where(is.character), as.factor))
```
...
1.  In the `fang_data` tibble, decompose the date as year, month, day, and weekday.Add them after the `date` column. Hint: Use the `year()`, `month()`, `day()`, and `wday()`.

2.  Add name of the Month and name of the day of the week. Hint: Read the documentation for `month()` and `wday()`.

# Solutions: Exercise 1

```{r}
mutate(fang_data, year = year(date), month = month(date), day = day(date), week_day = wday(date), .after = date)
```

# Solutions: Exercise 2

```{r}
mutate(fang_data, year = year(date), month = month(date, label=TRUE, abbr = FALSE), day = day(date), week_day = wday(date,label=TRUE, abbr = FALSE ), .after = date)
```

# summarize() - Sum up variables -

(create cool slide for this one) - slide 00313
...
```{r}
summarize(fang_data, avg_volume = mean(volume))
```
...
```{r}
summarize(fang_data, avg_volume = mean(volume))
summarize(fang_data, n_rows = n())
```

** Equivalent Code in Base R **

```{r}
mean(fang_data$volume)
nrow(fang_data)
```

# Sum up variables

`summarize(data, condition)` allows you to calculate summary statistics for particular variables. You can use summarize() in tandem with across() to calculate summary statistics for several variables at once.

```{r}
summarize(fang_data, across(where(is.numeric), mean))
```
...
```{r}
summarize(fang_data, across(c("volume", "adjusted"), ~mean(.x, na.rm = TRUE)))
```
...
```{r}
summarize(fang_data, across(where(is.numeric), list(average = mean, max=max)))
```
...
```{r}
summarize(fang_data, across(c("volume", "adjusted"), list(average = mean, max = max)))
```
...
1.  In the `fang_data` tibble, get the standard deviation, minimun, maximum of all numeric variables.

2.  In the `fang_data` tibble, compute the first quartile (25-percentile) and third quartile (75-percentile) for the `volume` and `high` columns. Hint: Use the quantile() function

# Solutions: Exercise 1

```{r}
summarize(fang_data, across(where(is.numeric), list(sd = sd, min = min, max = max)))
```

# Solutions: Exercise 2

```{r}
summarize(fang_data, volume_first_quartile = quantile(volume, probs = .25), volume_third_quartile = quantile(volume, probs = .75))

summarize(fang_data,
          across(c("volume", "high"),
                 list(
                   first_quartile = ~quantile(.x, probs = 0.25), 
                   third_quartile = ~quantile(.x, probs = .75)
                   )
                 )
          )
```
